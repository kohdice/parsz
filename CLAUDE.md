...
| `src/validator.zig`   | `validate` — converts `RawResult` → `ParseResult`. Type conversion (string to typed values: integers, floats, booleans, strings), required checks, default application. Generates `ParseResult` (comptime struct, typed fields). |
